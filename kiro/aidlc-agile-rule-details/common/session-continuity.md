...
3. **MANDATORY: Load Previous Stage Artifacts** - Before resuming any stage, automatically read all relevant artifacts from previous stages:
   - **Vision Stage**: Read vision-statement.md, stakeholder-map.md
   ...
   - **Epic Stage**: Read epics.md, epic-dependency.md, epic-story-map.md
   - **Backlog**: Read product-backlog.md, release-plan.md, backlog-history.md
   ...
   - **Jira Integration**: Read jira-config.md, jira-sync-log.md (if Jira MCP used)
4. **Smart Context Loading by Stage**:
   - **Early Stages (Workspace Detection, Vision Stage)**: Load workspace analysis, vision artifacts
   - **Requirements/Stories**: Load vision + reverse engineering + requirements artifacts
   - **Epic Stage**: Load requirements + stories artifacts
   - **Workflow Planning**: Load all inception artifacts + initialize backlog context
   - **Design Stages**: Load requirements + stories + epics + backlog + architecture + design artifacts
   - **Code Stages**: Load ALL artifacts + existing code files + current backlog status
...
10. **Backlog Status**: Always load current backlog status and include in context summary
11. **Jira Status**: If Jira MCP was configured, check current sync status
